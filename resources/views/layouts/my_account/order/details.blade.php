...
<div class="col-md-12">
	<ul class="col-md-12 "><h4 style="letter-spacing: 1px;font-weight: 400;font-size: 24px;">Order <a href="" style="color: #52d862;">#{{$order->id}}</a> Details</h4></ul>
</div>
		...
		<tr style="border-style: hidden;">
			...
			<td style="width: 30%" valign="top">
				...
				<b style="font-size: 20px;">Invoice:</b><br>
					<a class="" style="padding-left: 10px;" target="_blank" href="{{url('invoices')}}/{{$order->appoitment_id}}/download"><i class="fa fa-file-pdf-o" style="color: red"></i> invoice.pdf</a>
			</td>
		</tr>
	</table>
<table class="" cellspacing="0" cellpadding="0" border="0" style="border-style: hidden;">
	<tr>
		...
		<tr style="border-style: hidden;">
		<td colspan="2">Order status:  @if($order->is_paid==0) Unpaid @else Paid @endif   <a href="{{url('order/payment')}}/{{$order->id}}" title="" style="text-decoration: none;"> @if($order->is_paid==0)<i class="fa fa-credit-card"></i> PAY</a>@endif </td>
	</tr>
</table>
...
<table style="width: 100%" class="table" cellpadding="0" cellspacing="0">
	<tr>
		...
		<td style="width: 40%">
			...
			<li>Shipping cost : {{$order->shippment_cost}} <span>€</span></li>
			<li>Vat : {{$order->vat_amount}} <span>€</span></li>
			<li>Total : {{$order->sub_total}} <span>€</span></li>
			<li>Total with vat : {{$order->total}} <span>€</span></li>
			@if($order->is_wallet==1)
			<li>Wallet : {{$order->wallet_paid}} <span>€</span></li>
			@endif
