...
<div class="row">
	<form action="{{url('invoices/conframinvoice')}}" method="post" accept-charset="utf-8">
...
<div class="col-md-12">
	<ul class="col-md-6">
		<b>Client: <a href="{{url('clients')}}/{{$users->id}}/edit" class="fa fa-pencil"></a></b>
		...
		<address style="margin-top: -10px;">{{$users->email}}</address>
			...
			<span style="font-weight: bold;">Invoice address <a href="{{url('clients')}}/{{$users->id}}/edit" class="fa fa-pencil"></a></span>
		...
		<address class="col-md-6 row">
			<span style="font-weight: bold;">Shipping address <a href="{{url('clients')}}/{{$users->id}}/edit" class="fa fa-pencil"></a></span>
...
<h4>Notes</h4>
<textarea class="form-control" name="order_note" rows="4" style="background-color:#f7f5f5">{{$invoice->note}}</textarea>
</ul>
...
<table class="table" style="border-style: hidden;">
	<tr>
		...
		<td style="border-style: hidden;" align="left">
			<input type="text" name="shippment_cost" placeholder="0.00" style="border:1px solid #000;border-style: hidden;width: 60px" id="shippment_cost" onchange="withShipmentCost()" autocomplete="off" readonly="" value="{{$order->shippment_cost}}">
			<span>€</span>
		</td>
	</tr>
	<tr>
